...
export class SdrEffects {
  ...
  getRearchAge = createEffect(() => this.actions.pipe(
    ...
    switchMap((action: fromSdr.GetResearchAgeAction) =>
      this.repos
        .get(action.name)
        .getResearchAge(action.payload.query, action.payload.filters, action.payload.dateField, action.payload.accumulateMultivaluedDate, action.payload.upperLimitInYears, action.payload.groupingIntervalInYears)
        .pipe(
